...
func addRlimits(s *specgen.SpecGenerator, g *generate.Generator) {
	...
	for _, u := range s.Rlimits {
		...
		g.Config.Process.Rlimits = append(g.Config.Process.Rlimits, specs.POSIXRlimit{
			Type: name,
			Hard: u.Hard,
			Soft: u.Soft,
		})
	...
	// 		}
	// 	}
	// 	fmt.Printf("added RLIMIT_NOFILE max %d, curr %d\n", max, current)
	...
	// 		}
	// 	}
	// 	fmt.Printf("added RLIMIT_NPROC max %d, curr %d\n", max, current)
